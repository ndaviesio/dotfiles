...
sudo defaults write "com.apple.menuextra.clock" ShowDate -int 2

# Menu Bar Only > Clock Options > Show tyhe day of a week
sudo defaults write "com.apple.menuextra.clock" ShowDayOfWeek -bool false
...
if command -v dockutil >/dev/null 2>&1; then
    for item in \
        ...
        [ -e "${item}" ] && dockutil --add "${item}" --no-restart &> /dev/null
    done
